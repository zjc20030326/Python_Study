...
class Manage(PasswdMixin, LoggerMixin):
    ...
    def confirm_passwd(self):
        ...
        while not self.members.get(cardid).passwd == passwd:  # type: ignore
    ...
    def create_member(self):
        ...
        self.log(
            f"开卡成功：{self.cardid} -> {name}，时间：{time.strftime('%Y-%m-%d %H:%M:%S', regdate)}\n"
        )
        self.cardid += 1
    ...
    def modify_passwd(self):
        ...
        self.log(
            f"修改密码：卡号 -> {self.cardid}，时间：{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}\n"
        )
        print("密码修改成功。")
    ...
    def pay_money(self):
        ...
        self.log(
            f"积分累计：卡号 -> {self.cardid}，+{money}分，时间：{time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())}\n"
        )
