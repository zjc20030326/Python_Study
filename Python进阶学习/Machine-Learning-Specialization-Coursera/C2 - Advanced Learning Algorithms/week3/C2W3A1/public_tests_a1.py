...
def model_test(target, classes, input_size):
    # Keras 3 compatibility: use build() instead of relying on _inbound_nodes
    target.build((None, input_size))
    expected_lr = 0.01
    ...
    for layer in target.layers:
        ...
        assert list(layer.output_shape) == expected[i][1], \
            f"Wrong number of units in layer {i}. Expected {expected[i][1]} but got {list(layer.output_shape)}"
