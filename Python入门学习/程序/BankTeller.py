...
def main():
    ...
    def get_msg(num=False, name=False, passwd=False):
        ...
        if num:
            msg["num"] = get_integer_input("请输入卡号：")  # type: ignore
        if name:
            msg["name"] = input("请输入姓名：")  # type: ignore
        if passwd:
            msg["passwd"] = input("请输入密码：")  # type: ignore
        return msg
    ...
    while True:
        ...
        if ins == "1":
            ...
            bank.create_account(msg["name"], msg["passwd"], round(money, 2))  # type: ignore
